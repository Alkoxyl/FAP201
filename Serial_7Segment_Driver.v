module Scrolling_8Digit_7Seg_Driver (
    input wire i_Clk,
    ...
    reg [7:0] w_Text_Arr [0:31];
    integer i, k;
    always @(*) begin
        for (i = 0; i < 32; i = i + 1) w_Text_Arr[i] = BLANK;
    ...
    always @(posedge i_Clk) begin
        if (r_Scroll_Timer >= 24'd10_000_000) begin
            r_Scroll_Timer <= 0;
            if (r_Scroll_Index < 24)
    ...
    always @(*) begin
        if (i_Game_State == 3'd6 || i_Game_State == 3'd5) begin
            for (k = 0; k < 8; k = k + 1) begin
                r_Msg_Buffer[k] = w_Text_Arr[24 - r_Scroll_Index + k];
            end
        end else begin
            for (k = 0; k < 7; k = k + 1) r_Msg_Buffer[k] = BLANK;
            r_Msg_Buffer[7] = r_Score_Seg; 
        end
    end
    ...
    always @(posedge i_Clk) begin
        ...
        if (r_Clk_Div == 8'd0) begin
            if (r_Bit_Index < 16) begin
                ...
                r_Shift_Reg_16 <= {w_Digit_Select, r_Msg_Buffer[r_Digit_Idx]};
            end
        end 
